...
# 7. 역사적 거시경제 위기 타임라인 및 S&P 500 추이 비교 섹션
st.subheader("📉 역사적 거시경제 위기 사이클 & S&P 500 비교 (시계열 추이)")
st.markdown("1970년대 오일쇼크부터 현재까지 주요 경제 위기 충격 지수와 **미국 S&P 500 지수(참조 빨간선 기준)**의 장기 흐름을 비교하는 대시보드 그래프입니다[cite: 4].")

# 연도별 경제 충격 지수 및 S&P 500 장기 연동 데이터 세트 구축 (S&P 500은 빨간선 역할을 수행)
history_trend_data = pd.DataFrame({
    "연도(Year)": [1970, 1973, 1975, 1980, 1982, 1990, 1997, 2001, 2003, 2008, 2010, 2020, 2022, 2026],
    "경제 충격 지수": [45, 75, 80, 85, 70, 40, 75, 60, 50, 100, 55, 90, 75, 65],
    "S&P 500 지수 (참조선)": [100, 115, 75, 120, 140, 350, 970, 1140, 800, 1250, 1150, 3200, 3800, 5600]
}).set_index("연도(Year)")

# Streamlit 내장 line_chart는 다중 열을 지원하며, Altair 기반 커스텀 렌더링 또는 기본 멀티라인을 제공합니다.
st.line_chart(history_trend_data, use_container_width=True)

st.markdown("""
<div class="header-info-box">
    🔴 <b>그래프 참고:</b> 파란색 선은 역사적 경제 위기 및 충격 강도를 뜻하며, 함께 표시되는 지표선(S&P 500 지수)은 장기적인 미국 증시의 우상향 궤적 및 위기 국면별 반등 포인트를 대조하여 보여줍니다.
</div>
""", unsafe_allow_html=True)
